...
func (w *walletControllerImpl) GetWalletType(c *fiber.Ctx) error {
	walletType, err := w.walletService.GetWalletType(c.UserContext())
...
func (w *walletControllerImpl) GetWalletByUID(c *fiber.Ctx) error {
	userid := c.Get("userid")
	walletsByUserId, err := w.walletService.GetWalletByUserId(c.UserContext(), userid)
...
func (w *walletControllerImpl) AddWallet(c *fiber.Ctx) error {
	...
	wallet, err := w.walletService.AddWallet(c.UserContext(), &model.Wallet{
		UserId: userid,
		Name:   body.Name,
		Type:   body.Type,
		Amount: body.Amount,
...
func (w *walletControllerImpl) DeleteWallet(c *fiber.Ctx) error {
	...
	err = w.walletService.DeleteWallet(c.UserContext(), userid, uint32(walletId))
