...
const (
	GetUserAll = `select data.user_id, data.username, data.full_name, data.status, data.created_date, data.password, data.refresh_token, data.user_data
		from (select u.user_id, u.username, u.full_name, u.status, u.created_date, ua1.data_value as password, coalesce(ua2.data_value,'') as refresh_token, concat('{',string_agg(concat('"',ud.data_key,'":"',ud.data_value,'"'),','),'}') as user_data
		from public.users u
		...
		group by (u.user_id, u.username, u.full_name, u.status, u.created_date, password, refresh_token)) as data `

	GetUserByEmail = `select ud.user_id, u.username, u.full_name, u.status, u.created_date, ua1.data_value as password, coalesce(ua2.data_value,'') as refresh_token, concat('{',string_agg(concat('"',ud2.data_key,'":"',ud2.data_value,'"'),','),'}') as user_data
		...
		group by (ud.user_id, u.username, u.full_name, u.status, u.created_date, password, refresh_token)`
